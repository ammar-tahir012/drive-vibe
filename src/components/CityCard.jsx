...
const CityCard = ({ cityName, countryCode, countryName }) => {
  ...
  useEffect(() => {
    const fetchThumbnail = async () => {
      try {
        ...
        console.error(`CityCard [${cityName}]: Error:`, error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div 
      onClick={handleClick}
      className="group relative aspect-[3/4] md:aspect-[4/3] rounded-2xl overflow-hidden cursor-pointer bg-surface h-full transform transition-all duration-500 hover:scale-[1.03] hover:shadow-2xl border border-white/5"
    >
      {/* Thumbnail from verified YouTube video */}
      {thumbnail ? (
        <img 
          src={thumbnail} 
          ...
          onError={(e) => {
            // If even the hqdefault fails, try sddefault
            if (e.target.src.includes('hqdefault')) {
              e.target.src = e.target.src.replace('hqdefault', 'sddefault');
            }
          }}
        />
      ) : (
        <div className="w-full h-full bg-gradient-to-br from-surface to-surface3 flex items-center justify-center">
          {isLoading ? (
            ...
            <span className="text-white/20 text-xs font-body">No preview available</span>
          )}
        </div>
      )}
